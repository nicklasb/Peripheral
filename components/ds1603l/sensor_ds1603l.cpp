...
int init_ds1603l(char *log_prefix) {
    ...
    if (!sensorSerial) { // If the object did not initialize, then its configuration is invalid 
        ESP_LOGE(log_prefix, "Invalid SoftwareSerial pin configuration, check config");
    } 
    sensor.begin();   

    ESP_LOGI(log_prefix, "DS1603 sensor successfully initiated.");
